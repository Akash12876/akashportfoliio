    ...
    <section id="skills" className="relative w-full py-20 px-4 sm:px-8 bg-gradient-to-br from-[#232526] via-[#2c5364] to-[#0f2027] text-white flex flex-col items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl">
        {skillCategories.map((cat, i) => {
          // Alternate directions for 3D effect
          const directions = [
            { x: -120, y: 0, rotateY: -30 }, // left
            { x: 0, y: 120, rotateX: 30 },   // bottom
            { x: 120, y: 0, rotateY: 30 },  // right
            { x: 0, y: -120, rotateX: -30 }, // top
          ];
          const dir = directions[i % directions.length];
          return (
            <motion.div
              key={cat.title}
              initial={{ opacity: 0, ...dir, scale: 0.85 }}
              whileInView={{ opacity: 1, x: 0, y: 0, rotateY: 0, rotateX: 0, scale: 1 }}
              transition={{ duration: 0.85, delay: i * 0.18, ease: "backOut" }}
