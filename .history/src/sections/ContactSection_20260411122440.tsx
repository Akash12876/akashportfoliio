...
export default function ContactSection() {
  ...
  return (
    <section id="contact" className="w-full flex flex-col items-center py-10 px-1 sm:py-16 sm:px-2 bg-transparent relative overflow-hidden">
      ...
      <div className="hidden md:block absolute top-0 right-0 z-10 pr-8 pt-8">
        <IparxMediaLogo className="max-w-[180px] md:max-w-[200px] lg:max-w-[220px] w-full h-auto" />
      </div>
      <motion.div
        ...
        viewport={{ once: true }}
        className="w-full max-w-5xl rounded-2xl sm:rounded-3xl bg-black/80 border border-white/10 shadow-2xl backdrop-blur-lg flex flex-col md:flex-row p-3 xs:p-4 sm:p-8 md:p-14 gap-6 md:gap-0 relative md:ml-[-60px] lg:ml-[-100px]"
      ...
      {/* Mobile: logo centered below card */}
      <div className="block md:hidden w-full flex justify-center mt-6 xs:mt-8">
        <IparxMediaLogo className="max-w-[180px] xs:max-w-[220px] w-full h-auto" />
      </div>
      {/* Desktop: logo now at top right, handled above. */}
    </section>
  );
}
