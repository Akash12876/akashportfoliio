...
function ProjectImageWithFallback({ src, alt, idx }) {
  const [imgSrc, setImgSrc] = useState(src);
  const [errorCount, setErrorCount] = useState(0);
  if (errorCount > 1) {
    return (
      <div className="flex items-center justify-center w-[400px] h-[240px] bg-zinc-800 rounded-xl text-zinc-400 text-lg font-semibold">
        No Image
      </div>
    );
  }
  return (
    <Image
      ...
      priority={idx === 0}
      onError={() => {
        if (errorCount === 0) setImgSrc("/projects/fallback.png");
        setErrorCount((c) => c + 1);
      }}
    />
  );
