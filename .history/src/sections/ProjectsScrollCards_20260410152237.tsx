...
export default function ProjectsScrollCards() {
  ...
  const SECTION_HEIGHT = CARD_HEIGHT * projects.length;
  ...
  return (
    ...
    >
      {projects.map((project, idx) => {
        const top = `${(projects.length - idx - 1) * CARD_HEIGHT}vh`;
        return (
          <motion.div
            key={project.title}
            className="fixed left-1/2 -translate-x-1/2 w-full max-w-3xl"
            style={{
              top,
              zIndex: idx + 1,
            }}
