...
const skillCategories = [
  ...
  // All hooks and logic above, return at top level
  return (
    <section id="skills" ref={sectionRef} className="relative w-full py-32 px-4 sm:px-8 bg-gradient-to-br from-[#232526] via-[#2c5364] to-[#0f2027] text-white flex flex-col items-center min-h-[100vh]">
      <motion.h2
        ...
        <AnimatePresence initial={false} mode="wait">
          {skillCategories.map((cat, i) =>
            i === activeIndex ? (
              <motion.div
                key={cat.title}
                initial={{ opacity: 0, y: 120, scale: 0.96, rotateX: 18 }}
                animate={{ opacity: 1, y: 0, scale: 1, rotateX: 0 }}
                exit={{ opacity: 0, y: -120, scale: 0.96, rotateX: -18 }}
                transition={{ duration: 0.7, ease: "backOut" }}
                className="absolute left-0 right-0 mx-auto flex flex-col md:flex-row items-center justify-between bg-zinc-900/90 rounded-3xl shadow-2xl border border-white/10 p-8 md:p-12"
                style={{ minHeight: 420, zIndex: 10 }}
              >
                {/* Left: Icon grid */}
